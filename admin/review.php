...
// 權限檢查（只有管理員可以進入）
// 假設 requireAdminLogin() 存在於 iden.php 中，並用於檢查和強制跳轉未登入的用戶。
requireAdminLogin();
...
try {
    ...
    // 執行 SQL 查詢
    // 假設 fetchAll($sql, $params) 是您的資料庫操作函數
    ...
    // 顯示錯誤訊息，建議在實際生產環境中記錄到日誌而非直接輸出給用戶
    echo '<div class="container"><div class="alert alert-error">無法載入成果資料： ' . $e->getMessage() . '</div></div>';
}
...
<div class="container" style="padding-top: 40px; padding-bottom: 40px;">
    ...
    <div class="dashboard-stats" style="margin-bottom: 30px;">
        ...
        <div class="stat-card pending">
            ...
            <a href="?status=pending#review-list">查看列表 →</a> 
        </div>
        
        <div class="stat-card approved">
            <h3>已認證</h3>
            <div class="number"><?php echo $stats['approved']; ?></div>
            <a href="?status=approved#review-list">查看列表 →</a>
        </div>
        
        <div class="stat-card rejected">
            <h3>不通過</h3>
            <div class="number"><?php echo $stats['rejected']; ?></div>
            <a href="?status=rejected#review-list">查看列表 →</a>
        </div>
        
        <div class="stat-card">
            <h3>總數</h3>
            <div class="number"><?php echo $stats['total']; ?></div>
            <a href="review.php#review-list">查看全部 →</a>
        </div>
    ...
    <form method="GET" action="review.php" class="filter-bar" style="flex-wrap: wrap; margin-bottom: 30px;">
        ...
        <div style="flex-grow: 2; min-width: 200px;">
            ...
            <input
                ...
                placeholder="輸入學生姓名..."
                class="form-control"
                value="<?php echo $search_student; ?>"
            >
        </div>
        
        <div style="display: flex; gap: 10px; align-self: flex-end;">
            <button type="submit" class="btn btn-primary" style="margin-top: 0;" formaction="review.php#review-list">篩選</button>
            <a href="review.php" class="btn btn-secondary" style="margin-top: 0;">清除</a>
        </div>
    </form>


    <a id="review-list" style="display: block; margin-top: -80px; padding-top: 80px;"></a>
