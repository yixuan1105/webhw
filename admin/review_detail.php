<?php
// ==========================================
// admin/review_detail.php - 單筆成果檢視與審核表單
// ==========================================
    ...
    <?php if ($achievement): ?>
        ...
        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0"><?= htmlspecialchars($achievement['title']) ?></h4>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <h5>學生資訊</h5>
                        <p><strong>姓名：</strong> <?= htmlspecialchars($achievement['student_name']) ?></p>
                        <p><strong>帳號：</strong> <?= htmlspecialchars($achievement['username']) ?></p>
                        <p><strong>個人簡介：</strong> <?= nl2br(htmlspecialchars($achievement['bio'])) ?></p>
                    </div>
                    <div class="col-md-6">
                        <h5>成果資訊</h5>
                        <p><strong>類別：</strong> <?= htmlspecialchars($achievement['category']) ?></p>
                        <p><strong>上傳日期：</strong> <?= htmlspecialchars($achievement['upload_date']) ?></p>
                        <p><strong>目前狀態：</strong> <span class="<?= htmlspecialchars($achievement['status']) ?>"><?= ucfirst(htmlspecialchars($achievement['status'])) ?></span></p>
                    </div>
                </div>
                ...
                <div class="proof-box">
                    <?php if (!empty($achievement['proof_file'])): ?>
                        <p>
                            <a href="../uploads/<?= htmlspecialchars($achievement['proof_file']) ?>" target="_blank">
            ...
            <div class="card-body">
                <form method="POST" action="review_process.php">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="reviewer_comment" name="reviewer_comment" rows="3"><?= htmlspecialchars($achievement['reviewer_comment'] ?? '') ?></textarea>
